...
const EXPORT_OPTIONS = [
  {
    id: 'synthesis-pdf',
    label: 'Synthèse PDF',
    description: 'Tableau par étudiant : département, niveau, totaux et dernière demande.',
    icon: FileDown,
    format: 'pdf',
    scope: 'synthesis',
  },
  ...
  },
  {
    ...
    description: 'Une ligne par demande avec dates, statut et pièces jointes.',
    icon: Layers,
    format: 'xlsx',
    scope: 'detail',
  },
  {
    ...
    description: 'Historique complet des demandes filtrées, format paysage.',
    icon: List,
    format: 'pdf',
    scope: 'detail',
  },
...
export default function DemandeExportMenu({
  studentCount = 0,
  demandeCount = 0,
  filtersLabel = '',
  disabled = false,
      ...
      {open ? (
        ...
        >
          <div className="border-b border-light-gray bg-gradient-to-br from-slate-50 to-white px-4 py-3">
            <p className="text-xs font-bold uppercase tracking-wider text-esp-green">Exports demandes</p>
            <p className="mt-1 text-sm text-slate-700">
              <strong className="text-navy">{studentCount}</strong> dossier{studentCount > 1 ? 's' : ''}
              {' · '}
              <strong className="text-navy">{demandeCount}</strong> demande{demandeCount > 1 ? 's' : ''}
            </p>
            {filtersLabel ? (
