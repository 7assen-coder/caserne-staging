...
const EXPORT_OPTIONS = [
  {
    id: 'pdf',
    label: 'Registre PDF',
    description: 'Historique chronologique des entrées filtrées, format paysage.',
    icon: FileDown,
    format: 'pdf',
  },
  {
    ...
    description: 'Une ligne par entrée avec type, titre et auteur (.xlsx).',
    icon: FileSpreadsheet,
    format: 'xlsx',
  },
];

export default function JournalExportMenu({
  studentCount = 0,
  entryCount = 0,
  filtersLabel = '',
  disabled = false,
      ...
      {open ? (
        ...
        >
          <div className="border-b border-light-gray bg-gradient-to-br from-slate-50 to-white px-4 py-3">
            <p className="text-xs font-bold uppercase tracking-wider text-esp-green">Exports journal</p>
            <p className="mt-1 text-sm text-slate-700">
              <strong className="text-navy">{studentCount}</strong> dossier{studentCount > 1 ? 's' : ''}
              {' · '}
              <strong className="text-navy">{entryCount}</strong> entrée{entryCount > 1 ? 's' : ''}
            </p>
            {filtersLabel ? (
