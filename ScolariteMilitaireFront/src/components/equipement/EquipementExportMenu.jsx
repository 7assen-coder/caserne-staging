...
const EXPORT_OPTIONS = [
  {
    id: 'synthesis-pdf',
    label: 'Synthèse PDF',
    description: 'Tableau par étudiant : matricule, section, totaux en usage / rendu.',
    icon: FileDown,
    format: 'pdf',
    scope: 'synthesis',
  },
  ...
  },
  {
    ...
    description: 'Une ligne par pièce d\'équipement avec dates, état et pièce jointe.',
    icon: Layers,
    format: 'xlsx',
    scope: 'detail',
  },
  {
    ...
    description: 'Inventaire complet de toutes les pièces filtrées, format paysage.',
    icon: List,
    format: 'pdf',
    scope: 'detail',
  },
...
export default function EquipementExportMenu({
  studentCount = 0,
  itemCount = 0,
  filtersLabel = '',
  disabled = false,
  ...
  return (
    <div ref={rootRef} className="relative w-full sm:w-auto">
      ...
      >
        ...
        {!busy ? <ChevronDown size={14} className={`ml-1.5 opacity-70 transition ${open ? 'rotate-180' : ''}`} aria-hidden /> : null}
      </Button>
      ...
      {open ? (
        ...
        >
          <div className="border-b border-light-gray bg-gradient-to-br from-slate-50 to-white px-4 py-3">
            <p className="text-xs font-bold uppercase tracking-wider text-esp-green">Exports équipement</p>
            <p className="mt-1 text-sm text-slate-700">
              <strong className="text-navy">{studentCount}</strong> dossier{studentCount > 1 ? 's' : ''}
              {' · '}
              <strong className="text-navy">{itemCount}</strong> pièce{itemCount > 1 ? 's' : ''}
            </p>
            {filtersLabel ? (
