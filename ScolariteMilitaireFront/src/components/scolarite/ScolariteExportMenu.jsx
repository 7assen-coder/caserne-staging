...
const EXPORT_OPTIONS = [
  {
    id: 'synthesis-pdf',
    label: 'Synthèse PDF',
    description: 'Tableau par étudiant : département, niveau, statut et validations semestres.',
    icon: FileDown,
    format: 'pdf',
    scope: 'synthesis',
  },
  {
    ...
    description: 'Export .xlsx avec les colonnes semestres visibles.',
  ...
  },
  {
    ...
    description: 'Une ligne par semestre renseigné (format long).',
    icon: Layers,
    format: 'xlsx',
    scope: 'detail',
  },
...
export default function ScolariteExportMenu({
  studentCount = 0,
  columnCount = 0,
  filtersLabel = '',
  disabled = false,
  ...
  return (
    <div ref={rootRef} className="relative w-full sm:w-auto">
      ...
      >
        ...
        {busy ? 'Export en cours…' : 'Exporter le suivi'}
        {!busy ? (
          <ChevronDown size={14} className={`ml-1.5 opacity-70 transition ${open ? 'rotate-180' : ''}`} aria-hidden />
        ) : null}
      </Button>

      {open ? (
        <div
          className="absolute right-0 z-30 mt-2 w-[min(100vw-2rem,22rem)] overflow-hidden rounded-2xl border border-light-gray bg-white shadow-xl ring-1 ring-slate-200/80"
          role="menu"
        >
          <div className="border-b border-light-gray bg-gradient-to-br from-slate-50 to-white px-4 py-3">
            <p className="text-xs font-bold uppercase tracking-wider text-esp-green">Exports scolarité</p>
            <p className="mt-1 text-sm text-slate-700">
              <strong className="text-navy">{studentCount}</strong> dossier{studentCount > 1 ? 's' : ''}
              {' · '}
              <strong className="text-navy">{columnCount}</strong> col. semestres
            </p>
            {filtersLabel ? (
