...
export default function Header({ onMobileNavOpen }) {
  ...
  return (
    <header className="relative w-full min-w-0 border-b border-light-gray bg-white text-slate-900 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.9)]">
      <div
        className="pointer-events-none absolute inset-x-0 top-0 z-10 h-0.5 bg-gradient-to-r from-esp-red via-gold to-esp-green opacity-95"
        aria-hidden
      />
      ...
      <div className={rowClass}>
        ...
        <label className="relative min-w-0 flex-1 basis-0 md:max-w-none">
          ...
          <input
            ...
            className="h-10 w-full rounded-full border border-light-gray bg-white py-2 pl-10 pr-3 text-sm text-slate-900 shadow-inner shadow-slate-200/60 outline-none ring-0 transition placeholder:text-slate-400 focus:border-gold/60 focus:ring-2 focus:ring-gold/25 sm:h-11 sm:pl-11"
          />
        </label>

        <div
          className="pointer-events-none flex h-9 min-w-0 max-w-[min(100%,20rem)] shrink-0 select-none items-center gap-2 rounded-full border border-light-gray bg-off-white py-0.5 pl-0.5 pr-2.5 text-slate-900 shadow-sm sm:h-10 sm:max-w-[min(100%,24rem)] sm:pr-3 lg:max-w-[min(100%,28rem)] lg:pr-3.5"
