...
export default function AppNavbar({ mobileOpen, onMobileClose }) {
      ...
      {flatItems.map(({ to, icon: Icon, label }) => (
        <li key={to}>
          <NavLink
            ...
            onClick={onNavigate}
            className={({ isActive }) =>
              `flex w-full items-center justify-start gap-2 rounded-lg px-3 py-2 text-sm font-semibold transition ${
                light
                  ? isActive
                    ? 'bg-navy text-white shadow-sm ring-1 ring-navy-700'
  ...
  return (
    <>
      <aside className="hidden h-full w-72 shrink-0 flex-col border-r border-navy-800 bg-navy text-white md:flex">
        <div className="border-b border-navy-800 bg-navy px-5 py-5">
        ...
        <div className="border-t border-navy-800 p-4">
          <button
            ...
            className="w-full rounded-lg border border-white/20 bg-navy px-3 py-2.5 text-sm font-semibold text-white transition hover:bg-navy-800"
          >
            Déconnexion
          </button>
        </div>
