...
const EXPORT_OPTIONS = [
  {
    id: 'synthesis-pdf',
    label: 'Synthèse PDF',
    description: 'Tableau par étudiant : âge, groupe sanguin, totaux et dernière consultation.',
    icon: FileDown,
    format: 'pdf',
    scope: 'synthesis',
  },
  {
    ...
    description: 'Même vue synthétique, fichier .xlsx pour le suivi infirmerie.',
  ...
  },
  {
    ...
    description: 'Une ligne par consultation avec motif, avis et pièces jointes.',
    icon: Layers,
    format: 'xlsx',
    scope: 'detail',
  },
  {
    ...
    description: 'Historique complet des consultations filtrées, format paysage.',
    icon: List,
    format: 'pdf',
    scope: 'detail',
  },
...
export default function MedicalExportMenu({
  studentCount = 0,
  consultationCount = 0,
  filtersLabel = '',
  disabled = false,
      ...
      {open ? (
        ...
        >
          <div className="border-b border-light-gray bg-gradient-to-br from-slate-50 to-white px-4 py-3">
            <p className="text-xs font-bold uppercase tracking-wider text-esp-green">Exports médicaux</p>
            <p className="mt-1 text-sm text-slate-700">
              <strong className="text-navy">{studentCount}</strong> dossier{studentCount > 1 ? 's' : ''}
              {' · '}
              <strong className="text-navy">{consultationCount}</strong> consultation{consultationCount > 1 ? 's' : ''}
            </p>
            {filtersLabel ? (
