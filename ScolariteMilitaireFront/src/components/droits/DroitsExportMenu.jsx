...
const EXPORT_OPTIONS = [
  {
    id: 'pdf',
    label: 'Export PDF',
    description: 'Liste des droits du mois sélectionné, format paysage.',
    icon: FileDown,
    format: 'pdf',
  },
  {
    id: 'xlsx',
    label: 'Export Excel',
    description: 'Tableau .xlsx pour le suivi administratif.',
    icon: FileSpreadsheet,
    format: 'xlsx',
  },
];

export default function DroitsExportMenu({
  studentCount = 0,
  disabled = false,
  busy = null,
  onExport,
      ...
      {open ? (
        ...
        >
          <div className="border-b border-light-gray bg-gradient-to-br from-slate-50 to-white px-4 py-3">
            <p className="text-xs font-bold uppercase tracking-wider text-esp-green">Exports droits</p>
            <p className="mt-1 text-sm text-slate-700">
              <strong className="text-navy">{studentCount}</strong> étudiant{studentCount > 1 ? 's' : ''}
            </p>
          </div>
          <ul className="p-2">
