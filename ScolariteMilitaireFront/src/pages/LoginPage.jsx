...
export default function LoginPage() {
  const { isAuthenticated, login, bootstrapped, mustChangePassword } = useAuth();
  ...
  if (isAuthenticated) {
    return <Navigate to={mustChangePassword ? '/login/set-password' : '/dashboard'} replace />;
  }
  ...
  const validateClientOrSetErrors = () => {
    ...
    if (!email.trim()) {
      ...
      setEmailErr('Format invalide — utilisez une adresse @esp.mr');
      ok = false;
    }
    if (!password.trim()) {
      ...
      <main className="relative z-10 flex min-h-screen min-h-[100dvh] flex-col items-center justify-center px-4 sm:px-8 lg:px-12 pt-24 pb-20 sm:pt-28 sm:pb-24">
        <div className="w-full max-w-xl sm:max-w-2xl lg:max-w-3xl">
          ...
          <div className="rounded-3xl border border-white/[0.14] bg-white shadow-[0_24px_80px_-12px_rgba(0,0,0,0.45)] ring-1 ring-black/[0.06] overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-8 p-6 sm:p-10 lg:p-12 font-sans" noValidate>
              <div className="space-y-6 sm:space-y-7">
                {formErr ? (
                  <p
                    className="text-base sm:text-lg font-medium text-brand-red bg-red-50 border border-red-100 rounded-xl px-4 py-3.5"
                    role="alert"
                  >
                    {formErr}
                  </p>
                ...
                <p className="text-base sm:text-lg text-text-light leading-snug">
                  Espace personnel — accès sécurisé par e-mail institutionnel.
                </p>
                ...
                <label className="form-field-contained block">
                  ...
                  <input
                    ...
                    onBlur={() => {
                      const v = email.trim();
                      if (!v) return;
                      if (!isEspEmail(v)) setEmailErr('Format invalide — utilisez une adresse @esp.mr');
                    }}
                ...
                <label className="form-field-contained block">
                  <span className="login-card-field-label flex flex-wrap items-center justify-between gap-2">
                    ...
                    <Link
                      to="/login/recovery"
                      className="text-sm sm:text-base font-semibold normal-case tracking-normal text-navy hover:text-brand-red underline-offset-2 hover:underline"
                    >
                      Mot de passe oublié ?
                    </Link>
                  </span>
                ...
                <label className="flex cursor-pointer select-none items-center gap-4 py-1">
                  <input
                    type="checkbox"
                    className="h-5 w-5 sm:h-6 sm:w-6 shrink-0 rounded border-light-gray text-navy focus:ring-navy/25"
                    checked={rememberMe}
