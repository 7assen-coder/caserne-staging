...
class Phase25HealthTests(TestCase):
    ...
    def test_render_yaml_uses_healthz_not_docs(self):
        ...
        # Host: …/esp_management/backend/backend/tests/… → repo root.
        # Docker test image: /code/backend/tests/… → mount render.yaml under /code.
        render = None
        for parent in Path(__file__).resolve().parents:
            candidate = parent / 'render.yaml'
            if candidate.is_file():
                render = candidate
                break
        self.assertIsNotNone(render, 'render.yaml not found walking up from test file')
